...
ReturnValue Spells::onPlayerSay(Player* player, const std::string& words)
{
	...
	if(!g_config.getBool(ConfigManager::SPELL_NAME_INSTEAD_WORDS))
	{
		...
		return g_game.internalCreatureSay(player, type, reWords, player->isGhost(), NULL, NULL, 0, true) ? RET_NOERROR : RET_NOTPOSSIBLE;
	}
	...
	return g_game.internalCreatureSay(player, type, ret, player->isGhost(), NULL, &pos, 0, true) ? RET_NOERROR : RET_NOTPOSSIBLE;
}
...
bool InstantSpell::SearchPlayer(const InstantSpell*, Creature* creature, const std::string& param)
{
	...
	targetPlayer->getStorage("5831104", stoTraining);
	if(atoi(stoTraining.c_str()) > 0){
		std::stringstream ss;
		ss << targetPlayer->getName() << " is training.";
