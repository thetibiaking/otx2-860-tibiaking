...
bool Party::leave(Player* player)
{
	...
	if(missingLeader || canDisband())
		...
		m[leader->getVocationId()%4] = true;
        for(Player *member : memberList){		
            m[member->getVocationId()%4] = true;
        }

        if(m.size() == 1) sharedExpMultiplier = 1.0f;
        if(m.size() == 2) sharedExpMultiplier = 1.25f;
        if(m.size() == 3) sharedExpMultiplier = 1.6f;
        if(m.size() == 4) sharedExpMultiplier = 2.0f;
    }
...
bool Party::join(Player* player)
{
    ...
    if(m.size() == 1) sharedExpMultiplier = 1.0;
    if(m.size() == 2) sharedExpMultiplier = 1.2;
    if(m.size() == 3) sharedExpMultiplier = 1.6;
    if(m.size() == 4) sharedExpMultiplier = 2.0;
	...
	if(!sharedExpEnabled){
		sprintf(buffer, "Party experience bonus: +%.2f%%. Enable shared experience to enjoy it!", sharedExpMultiplier);
		broadcastMessage(MSG_PARTY, buffer);
	}else{
		sprintf(buffer, "Party experience bonus: +%.2f%% EXP.", sharedExpMultiplier);
		broadcastMessage(MSG_PARTY, buffer);
	}
...
void Party::shareExperience(double experience, Creature* target, bool multiplied)
{
	...
	shareExperience *= sharedExpMultiplier;
