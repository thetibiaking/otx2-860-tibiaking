...
bool ChatChannel::talk(Player* player, MessageClasses type, const std::string& text, uint32_t statementId)
{
	...
	if (channelId == 2 || channelId == 6 || channelId == 9) {
		if (player->getGroupId() == 6) {
			...
			//ntype = MSG_PRIVATE; manda msg para todos tipo privado
		}
...
ChatChannel* Chat::createChannel(Player* player, uint16_t channelId)
{
	...
	switch(channelId)
	{
		...
		case CHANNEL_PRIVATE:
		{
			...
			if(getPrivateChannel(player)) //!player->isPremium() ||
				return NULL;
...
ChatChannel* Chat::addUserToChannel(Player* player, uint16_t channelId)
{
	...
	if (channelId == CHANNEL_LOOT) {
		ChatChannel* newChannel = new ChatChannel(channelId, "Loot", ChatChannel::staticFlags);
		if (newChannel) {
...
bool Chat::removeUserFromChannel(Player* player, uint16_t channelId)
{
	...
	ChatChannel* checkChannel = getChannel(player, channelId);
    if (channelId == CHANNEL_LOOT && checkChannel) {
		deleteChannel(player, channelId);
	}
...
ChannelsList Chat::getChannelList(Player* player)
{
	...
	list.push_back(std::make_pair(CHANNEL_LOOT, "Loot"));
...
ChatChannel* Chat::lootChannelOpen(Player* player)
{
	if (!player || player->isRemoved()) {
		return NULL;
	}
