...
void Player::sendCancelMessage(ReturnValue message) const
{
	switch(message)
	{
		...
		case RET_HOUSEPROTECTED:
			sendCancel("House Protection is active!");
			break;
...
double Player::getGainedExperience(Creature* attacker) const
{
	...
	if(guildId == 0 || attackerPlayer->getGuildId() == 0 || !guildId || !attackerPlayer->getGuildId() || 
		guildId == attackerPlayer->getGuildId() || (party && party->isPlayerMember(attackerPlayer)) /*|| isEnemy(attackerPlayer, false)*/)
		return 0;

	// Verifica se o Player Pode ganhar EXP, ou se já matou mais de 3x o mesmo cara (pvpGuildEnforced.lua)
	// Caso a storage receba 1, ele vai liberar
	std::string enableExp;
    attackerPlayer->getStorage("720147", enableExp);
    if(atoi(enableExp.c_str()) < 0){
    	return 0;
    }
...
bool Player::gainExperience(double& gainExp, Creature* target)
{
	...
	if(gainExp >= level)
	{
		if(Condition* condition = Condition::createCondition(
			CONDITIONID_DEFAULT, CONDITION_SOUL, 2 * 60 * 1000))
		{
...
bool Player::rateExperience(double& gainExp, Creature* target)
{
	...
	// Bonus Cast
	double bonusExp = 0;
	std::string bonusCast;
	if (g_config.getNumber(ConfigManager::CAST_BONUS_EXP) != 0){
	    getStorage("1013121", bonusCast);
	    if(atoi(bonusCast.c_str()) == 1){
	    	bonusExp += g_config.getNumber(ConfigManager::CAST_BONUS_EXP);
	    }
    }

    // Bonus Custom Client
    if (g_config.getNumber(ConfigManager::CLIENT_BONUS_EXP) != 0 && getClientVersion() > 860){
    	bonusExp += g_config.getNumber(ConfigManager::CLIENT_BONUS_EXP);
    }

    if (bonusExp != 0) {
    	gainExp += gainExp*(bonusExp/100);
    }
...
bool Player::addUnjustifiedKill(const Player* attacked, bool countNow)
{
	...
	//Guild Frag System:
	if(guildId != 0 ){
		f = g_config.getNumber(ConfigManager::RED_GUILD_LIMIT), s = g_config.getNumber(
			ConfigManager::RED_SECOND_GUILD_LIMIT), t = g_config.getNumber(ConfigManager::RED_THIRD_GUILD_LIMIT);
	}
	if(countNow)
	{
		uint32_t fragsToday;
		if(skull < SKULL_RED)
			...
			fragsToday = g_config.getNumber(ConfigManager::BLACK_LIMIT);

		char buffer[90];
		sprintf(buffer, "Warning! The murder of %s was not justified.\n(Frags today: %d/%d)", attacked->getName().c_str(), fc, fragsToday);
...
void Player::broadcastPlayerLoot(const std::string& monster, const std::string items)
{
	std::stringstream s;
	s << "Loot of " << monster << ": " << items;

	s << ".";
	sendChannelMessage("", s.str().c_str(), MSG_CHANNEL_MANAGEMENT, CHANNEL_LOOT);
}