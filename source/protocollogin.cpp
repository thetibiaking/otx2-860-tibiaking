...
void ProtocolLogin::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	if(account.name == "10")
	{
		...
		{
			if(!it->second->isRemoved() && it->second->client->isBroadcasting() && it->second->getClientVersion() <= version)
		...
		for(PlayerVector::iterator it = players.begin(); it != players.end(); ++it)
		{
			...
			DBResult_ptr viewers;
			DBQuery query;
			std::string viewersStr;
			
			query << "SELECT `viewers` FROM `players` WHERE `id` = " << (*it)->getGUID() << ";";
			if(!(viewers = db->storeQuery(query.str())))
				viewersStr = "0";
			else
				viewersStr = viewers->getDataString("viewers");
