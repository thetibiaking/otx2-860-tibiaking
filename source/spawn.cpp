...
bool Spawns::parseSpawnNode(xmlNodePtr p, bool checkDuplicate)
{
	...
	//Obtendo o Nome da Criatura
	DBResult_ptr monsterNameBonus;
	DBQuery queryMonsterName;	

	std::string nameBonusCreature;
	std::string intervalBonus;
	int32_t intervalBonustoInt = 1000;

	queryMonsterName << "SELECT `value` FROM `global_storage` WHERE `key` = 308700;";
	

	if(!(monsterNameBonus = db->storeQuery(queryMonsterName.str())))
		nameBonusCreature = "nothing";
	else{
		nameBonusCreature = monsterNameBonus->getDataString("value");

		//Obtendo o interval
		DBResult_ptr monsterInterval;
		DBQuery queryMonsterInterval;	
		queryMonsterInterval << "SELECT `value` FROM `global_storage` WHERE `key` = 308701;";
		if(!(monsterInterval = db->storeQuery(queryMonsterInterval.str())))
			intervalBonustoInt = 1000;
		else{
			intervalBonus = monsterInterval->getDataString("value");
			intervalBonustoInt = std::atoi(intervalBonus.c_str());;
		}
	...
	for(xmlNodePtr tmpNode = p->children; tmpNode; tmpNode = tmpNode->next)
	{
		if(!xmlStrcmp(tmpNode->name, (const xmlChar*)"monster"))
		{
			...
			if (name == nameBonusCreature){
				interval *= intervalBonustoInt;
			}
			else{			
				interval *= 1000;
			}
...
void Spawn::checkSpawn()
{
	...
	for(SpawnMap::iterator it = spawnMap.begin(); it != spawnMap.end(); ++it)
	{
		...
		Database* db = Database::getInstance();
		DBResult_ptr result;
		std::string monsterAllowSpawn = "false";

		DBQuery query;
		query << "SELECT `value`, `value` FROM `global_storage` WHERE `key` = 308801;";
		if((result = db->storeQuery(query.str())))
		{
			monsterAllowSpawn = result->getDataString("value");
		}


		if(g_config.getBool(ConfigManager::ALLOW_BLOCK_SPAWN) && findPlayer(sb.pos) && monsterAllowSpawn != "1")
		{
