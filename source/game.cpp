	...
	uint32_t spriteId/* = 0*/, stackposType_t type/* = STACKPOS_NORMAL*/)
{
	...
	else if(!pos.y && !pos.z)
	{
		...
		if(!it.id)
			return NULL;
		if(it.id == 6499)
			player->sendTextMessage(MSG_STATUS_SMALL, "teste.");
...
bool Game::playerOpenChannel(uint32_t playerId, uint16_t channelId)
{
	...
	if(channelId == CHANNEL_LOOT) player->setGetLoot(true); 
...
bool Game::playerCloseChannel(uint32_t playerId, uint16_t channelId)
{
	...
	if(channelId == CHANNEL_LOOT) player->setGetLoot(false); 
...
bool Game::playerRequestOutfit(uint32_t playerId) //quando solicita a troca do outfit
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;
	...
	//player->sendTextMessage(MSG_STATUS_SMALL, "Aqui abre a caixa de troca de outfit.");
	return true;
}

bool Game::playerChangeOutfit(uint32_t playerId, Outfit_t outfit) //quando vc toca o outfit dentro da caixinha
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;
	...
	//player->sendTextMessage(MSG_STATUS_SMALL, "Eu Troquei Meu OUTFIT.");
	return true;
}
...
std::string _text = asLowerCaseString(text);
   ...
   std::string words []= {".ddns", "ilussion"};
   int ii, length;
   ...
   for(ii=0; ii < int(length); ii++)
   {
      if (int(_text.find(words[ii])) > 0 || _text == words[ii])
		{
			internalCreatureSay(player, MSG_SPEAK_SAY, "NewTibia, o melhor servidor da atualidade.", false, NULL); 
             return false;
          break;
		}
	}
	...
	const std::string& text, uint32_t statementId)
{
    ...
    if (player->getLevel() < 60) {
        player->sendTextMessage(MSG_STATUS_SMALL, "You must be level 60 to send private messages.");
        return false;
    }*/
	...
	for(it = list.begin(); it != list.end(); ++it)
    {
        ...
        if(!ghostMode || tmpPlayer->canSeeCreature(creature))
        {
            if(isSpell)
            {
                // to jest spell
                std::string strValue;
                if(tmpPlayer->getStorage("586780", strValue))
                {
...
{//nao achei o sentido dela ainda
	if(!forced)
	{
		bool deny = false;
...
bool Game::playerSharePartyExperience(uint32_t playerId, bool activate)
{
	...
	if(!player->getParty() || (!player->hasFlag(PlayerFlag_NotGainInFight)
		&& player->hasCondition(CONDITION_INFIGHT) && !player->getTile()->hasFlag(TILESTATE_PROTECTIONZONE)))
		return false;
...
bool Game::showHotkeyUseMessage(Player* player, Item* item)
{
	...
	if(Condition* showHotkeyExhaust = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_EXHAUST, 1300, 0, false, 4597))
        player->addCondition(showHotkeyExhaust);

	std::string msgType;
    player->getStorage("621457", msgType);
    
    if (atoi(msgType.c_str()) < 0 || count <= 25)
