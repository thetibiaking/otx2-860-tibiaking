...
void MonsterType::dropLoot(Container* corpse)
{
	...
	else if(!owner->getLoot() && (message == LOOTMSG_PLAYER || message == LOOTMSG_BOTH))
		owner->sendTextMessage((MessageClasses)g_config.getNumber(ConfigManager::LOOT_MESSAGE_TYPE), ss.str());

	if((owner->getLoot()))
		owner->broadcastPlayerLoot(nameDescription, corpse->getContentDescription());
}
...
bool Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading/* = false*/)
{
	...
	if(readXMLInteger(root, "experience", intValue)){
		...
		DBResult_ptr monsterBonus;
		DBResult_ptr monsterBonusName;
		DBQuery queryBonus;	
		DBQuery queryName;

		std::string bonusExp;
		std::string monsterNameBonus;
		float bonusExpToFloat = 1;

		
		queryName << "SELECT `value` FROM `global_storage` WHERE `key` = 308702;";
		if (monsterBonusName = db->storeQuery(queryName.str())){
			monsterNameBonus = monsterBonusName->getDataString("value");

			if (mType->name == monsterNameBonus) {
				queryBonus << "SELECT `value` FROM `global_storage` WHERE `key` = 308703;";
				if(monsterBonus = db->storeQuery(queryBonus.str()))
				{
					bonusExp = monsterBonus->getDataString("value");
					bonusExpToFloat = std::stof(bonusExp.c_str());;
					mType->experience = intValue*bonusExpToFloat;
				}
			}
		}
	}
	...
	for(xmlNodePtr p = root->children; p; p = p->next)
	{
		...
		else if(!xmlStrcmp(p->name, (const xmlChar*)"loot"))
		{
			for(xmlNodePtr tmpNode = p->children; tmpNode; tmpNode = tmpNode->next)
			{
				...
				// Bonus Loot System:
				Database* db = Database::getInstance();
				DBResult_ptr monsterBonusLoot;
				DBQuery query;


				query << "SELECT `value` FROM `global_storage` WHERE `key` = 308704;";
				if (monsterBonusLoot = db->storeQuery(query.str())){
					if (mType->name == monsterBonusLoot->getDataString("value")) {
						if(loadLoot(tmpNode, rootBlock, mType->name))
...
bool Monsters::loadLoot(xmlNodePtr node, LootBlock& lootBlock, const std::string& monsterName)
{
	std::string strValue;
	if(readXMLString(node, "id", strValue) || readXMLString(node, "ids", strValue))
	{
		...
		for(IntegerVec::iterator it = idsVec.begin(); it != idsVec.end(); ++it)
		{
			...
			if (monsterName != ""){
				...
				loadChildLoot(node, lootBlock, monsterName);
			}
			else{
				if(Item::items[(*it)].isContainer())
	...
	if(readXMLInteger(node, "chance", intValue) || readXMLInteger(node, "chance1", intValue)){		
		//Bonus Loot by Global Storage:
		float bonusChanceToFloat = 1;
		if (monsterName != ""){
			Database* db = Database::getInstance();
			DBResult_ptr monsterBonus;
			DBQuery queryBonus;	
			std::string bonusChance;			

			queryBonus << "SELECT `value` FROM `global_storage` WHERE `key` = 308705;";
			if(monsterBonus = db->storeQuery(queryBonus.str()))
			{
				bonusChance = monsterBonus->getDataString("value");
				bonusChanceToFloat = std::stof(bonusChance.c_str());
				if (bonusChanceToFloat > 0){	
					intValue*=bonusChanceToFloat;
				}						
			}
...
bool Monsters::loadChildLoot(xmlNodePtr node, LootBlock& parentBlock, const std::string& monsterName)
{
	if(!node)
		return false;
	...
	for(xmlNodePtr p = node->children; p; p = p->next)
	{
		if(xmlStrcmp(p->name, (const xmlChar*)"inside"))
		{
			...
			if (monsterName != ""){
				if(loadLoot(p, childBlock, monsterName))
		...
		for(xmlNodePtr insideNode = p->children; insideNode; insideNode = insideNode->next)
		{
			LootBlock childBlock;
			if (monsterName != ""){
				if(loadLoot(insideNode, childBlock, monsterName))
					parentBlock.childLoot.push_back(childBlock);
			}
			else{
