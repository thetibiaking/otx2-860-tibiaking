...
function getGuildPlayersValidAccIDS(GuildID, MinLevel)
        ...
        Query3 = db.getResult("SELECT `id` FROM `accounts` WHERE `guild_bonus_stats` = 0 AND `id` IN (".. table.concat(AccsID, ', ') ..")")
                        ...
                        if #GuildMembersOnline >= GuildPointsConfigs.NeedPlayersOnline then
                                ...
                                if #IPS >= GuildPointsConfigs.NeedDiferentIps then
                                        ...
                                        if #ValidAccounts > 0 then
                                                db.executeQuery("UPDATE `accounts` SET `premium_points` = `premium_points` + " ..GuildPointsConfigs.AddPointsForAcc .. ", `guild_bonus_stats` = ".. os.time() .." WHERE `id` IN (" .. table.concat(ValidAccounts, ',') ..");")
                                                for i, pid in ipairs(GuildMembersOnline) do
                                                        local PlayerMSGAccID = getPlayerAccountId(pid)
                                                        for k = 1, #ValidAccounts do
                                                                if PlayerMSGAccID == ValidAccounts[k] then
                                                                        doPlayerSendTextMessage(pid, MESSAGE_INFO_DESCR, "You received "..GuildPointsConfigs.AddPointsForAcc .." Premium Points.")
                                                                        break
                                                                end
                                                        end
                                                end
