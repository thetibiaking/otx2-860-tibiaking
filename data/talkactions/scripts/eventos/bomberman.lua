function onSay(player, words, param)
	if not table.contains(BomberTeam1, player) and not table.contains(BomberTeam2, player) then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "So e permitido soltar bombas dentro e durante a partida")
	return true
	end
	
	if player:getStorageValue(Storage.bomberman.activebombs) < player:getStorageValue(Storage.bomberman.maxbombammount) then
		local bombsize = player:getStorageValue(Storage.bomberman.bombsize)
		local item = Game.createItem(26142, 1, player:getPosition())
		player:setStorageValue(Storage.bomberman.activebombs, player:getStorageValue(Storage.bomberman.activebombs) + 1)
...
function checktile(position)
	...
	if creature and creature:isPlayer() then
		...
		if creature:getStorageValue(Storage.bomberman.bombsize) > 1 or creature:getStorageValue(Storage.bomberman.maxbombammount) > 1 or creature:getStorageValue(Storage.bomberman.bombermanspeed) > 1 then
			creature:setStorageValue(Storage.bomberman.maxbombammount, 1)
			creature:setStorageValue(Storage.bomberman.bombsize, 1)
			creature:setStorageValue(Storage.bomberman.bombermanspeed, 1)
			doChangeSpeed(creature, getCreatureBaseSpeed(creature)-creature:getSpeed())
			creature:sendTextMessage(MESSAGE_INFO_DESCR, "Voce foi atingido, e perdeu suas habilidades.")
		else
		-- segundo hit: Se nao tiver powerUps, sai da partida
			...
			creature:sendTextMessage(MESSAGE_INFO_DESCR, "Voce foi atingido, e morreu por estar sem habilidades.")
	...
	if position.x >= 1009 and position.x <= 1036 and position.y >= 905 and position.y <= 929 then 
		position:sendMagicEffect(CONST_ME_FIREAREA)
	end
end