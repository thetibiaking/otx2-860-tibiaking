...
function onThink(cid, interval)
	...
	if getTilePzInfo(getCreaturePosition(cid)) and getPlayerStorageValue(cid, storage) <= 0 then
		...
		if getPlayerStorageValue(cid, store) - os.time() <= 0 then
			doPlayerSendTextMessage(cid, 20, "{Stamina Recovery} Sua stamina começou a se recuperar, o fator de recuperação é de 1 minuto de stamina a cada 7 minutos dentro da zona de proteção em relação as condições normais.")
