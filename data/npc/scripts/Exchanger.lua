...
function creatureSayCallback(cid, type, msg)
	if(not npcHandler:isFocused(cid)) then
		...
		if msgcontains(msg, 'offer') or msgcontains(msg, 'job') then
...
selfSay(item, cid)
		...
		elseif msgcontains(msg, 'warrior\'s sweat') or msgcontains(msg, 'warrior sweat') then
...
talk_state = 0
		...
		elseif msgcontains(msg, 'lottery ticket') or msgcontains(msg, 'lotto ticket') or msgcontains(msg, 'ticket') then
				selfSay('If you want to buy a lotto ticket, just ask me to {trade}', cid)
				talk_state = 0
