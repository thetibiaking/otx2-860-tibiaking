...
function creatureSayCallback(cid, type, msg)
    ...
    if (not npcHandler:isFocused(cid)) then
        ...
        elseif msgcontains(msg, 'yes') and talk_state == 1 then
                local price = 1000
            if getPlayerItemCount(cid, 2386) >= 1 and getPlayerItemCount(cid, 10305) >= 1  and getPlayerMoney(cid) >= price then
                ...
                npcHandler:say('I need an {axe}, {holy icon} and ' .. price .. ' gold coins to make you a {hallowed} {axe}.', cid)
                talk_state = 0
            end
