	...
	while k < range and (getCreaturePosition(uid).x ~= position.x or getCreaturePosition(uid).y ~= position.y) do	
		...
		addEvent(function()
			if isCreature(uid) then
				...
				if isWalkable(toPos, true, true, false) then
					...
					for i = 1, #getPositionsByDirection(uid) do
						...
						if isWalkable(getPositionsByDirection(uid)[i], true, true, false) and getDistanceBetween(getPositionsByDirection(uid)[i], position) <= getDistanceBetween(interPos, position) then
