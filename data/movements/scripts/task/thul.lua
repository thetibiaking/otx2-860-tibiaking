...
local thaiss = {x=32513, y=32531, z=7}
    ...
    if item.actionid == 16355 and getPlayerStorageValue (cid, 75311) == 1 and getPlayerStorageValue (cid, 17667) == -1 then 
		...
		doPlayerSetStorageValue (cid, 17667, 1)
		doSummonCreature(config.boss, config.bosspos[1])
		elseif getPlayerStorageValue (cid, 75311) == -1 then
		doTeleportThing(cid,thaiss)
		doCreatureSay(cid, 'You did not complete the task Quest!', TALKTYPE_ORANGE_1)
		else
		doTeleportThing(cid,thaiss)
		doCreatureSay(cid, 'You already had a chance to kill Thul.', TALKTYPE_ORANGE_1)
		end	
    return 1
end 
