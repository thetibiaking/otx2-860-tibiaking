...
local blockingTree =
{
        ...
        if blockingTree[itemEx.itemid] and itemEx.uid == blockingTree[itemEx.itemid][1] then
               ...
               if getPlayerStorageValue(cid, storages.strTask) <= 0 then
                        doPlayerSendCancel(cid, "You need complete Demons Task.")
                        return TRUE
                end
        ...
        elseif isInArray(demonOak, itemEx.itemid) then
                ...
                for i = 1, #summons[get] do
                        doCreateMonster(summons[get][i], positions.summon[i], false, true)
                end
