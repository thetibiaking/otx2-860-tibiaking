function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	local asuriCracksA = {x = 32811, y = 32707, z = 10}
	if(itemEx.uid == 35009) and (itemEx.itemid == 6299) then
				doTeleportThing(cid, asuriCracksA, TRUE)
                doSendMagicEffect(asuriCracksA, CONST_ME_TELEPORT)
                doSendMagicEffect(fromPosition, CONST_ME_POFF)
        return TRUE
		end
		
	local asuriCracksB = {x = 32871, y = 32722, z = 10}
	if(itemEx.uid == 35010) and (itemEx.itemid == 6299) then
				doTeleportThing(cid, asuriCracksB, TRUE)
                doSendMagicEffect(asuriCracksB, CONST_ME_TELEPORT)
                doSendMagicEffect(fromPosition, CONST_ME_POFF)
        return TRUE
		end
	
	local asuriCracksC = {x = 32865, y = 32778, z = 10}
	if(itemEx.uid == 35011) and (itemEx.itemid == 6299) then
				doTeleportThing(cid, asuriCracksC, TRUE)
                doSendMagicEffect(asuriCracksC, CONST_ME_TELEPORT)
                doSendMagicEffect(fromPosition, CONST_ME_POFF)
        return TRUE
		end
		
	local asuriCracksD = {x = 32829, y = 32779, z = 10}
	if(itemEx.uid == 35012) and (itemEx.itemid == 6299) then
				doTeleportThing(cid, asuriCracksD, TRUE)
                doSendMagicEffect(asuriCracksD, CONST_ME_TELEPORT)
                doSendMagicEffect(fromPosition, CONST_ME_POFF)
        return TRUE
		end
...
doItemSetAttribute(kurwos, "aid", 15106)
	...
	if toPosition.x == 32551 and toPosition.y == 31379 and toPosition.z == 15 and 129 < getPlayerLevel(cid) then
